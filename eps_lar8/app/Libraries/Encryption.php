...
class Encryption
{
	...
	public static function is_php($version = '7.0.0')
    {
	...
	public function __construct(array $params = array())
	{
		...
		isset(self::$func_overload) OR self::$func_overload = ( ! self::is_php('8.0') && extension_loaded('mbstring') && @ini_get('mbstring.func_overload'));
		$this->initialize($params);

		if (!isset($this->_key) && self::strlen($key = 'mms-encryption-key') > 0) {
			$this->_key = $key;
		}
		

		Log::info('Encryption Class Initialized');
	}
	...
	public function initialize(array $params)
	{
		...
		if (empty($this->_driver))
		{
			...
			error_log("DEBUG - Encryption: Auto-configured driver '".$this->_driver."'.");
		}
	...
	protected function _openssl_initialize($params)
	{
		...
		if (isset($this->_cipher, $this->_mode))
		{
			...
			else
			{
				...
				error_log("INFO - Encryption: OpenSSL initialized with method '".strtoupper($handle)."'.");
			}
		}
	}
